...
## 0. 1分要約
- **目的**: MVPから成長期まで大きな作り直しを避ける設計
- **構成**: App Router + features/shared/external の層分離
- **強み**: External層でDB/API切替、RSC+Server Actionsで安定したデータフロー
- **認証**: Next.js側に集約（BFF）し、UIは認証済み前提で動作

## 用語の簡単な説明
- **RSC**: React Server Component。サーバーで描画されるコンポーネント
- **Server Action**: クライアントから呼び出せるサーバー関数
- **DTO**: 入出力の契約を固定する型（API/DBの変更に強い）

## 1. 目的・対象読者
採用担当（非技術）とエンジニアの双方に向けて、本アプリのフロントエンド設計の
考え方と構成を短時間で理解できるようにまとめた資料です。
内容は `frontend/docs`（01〜07）に基づいています。
...
`frontend/docs/01-tech-stack.md` の内容を詳細化しています。
